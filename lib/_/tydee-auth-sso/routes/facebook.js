...
import api, { tydee, facebook } from '../server';
...
export default class extends Component {
    ...
    renderScene() {
        return (
            <View style={[styles.webViewContainer, styles.webViewContainerFacebook]}>
                <WebView
    ...
    async saveResponse() {
        try {
            await AsyncStorage.setItem(tydee.key, JSON.stringify(this.state.response));
        }
        catch (error) {
            this.onError(error);
        }
    ...
    async removeResponse() {
        try {
            await AsyncStorage.removeItem(tydee.key);
